...
import glob
from CMI_utils import get_network, get_premodel
...
class Synthesizer():
    ...
    def loader(self, args, augment=True):
        ...
        self.data_dec = train_transform(data_dec)
    ...
    def test(self, args, val_loader, logger, bench=True):
        ...
        test_data(args, loader, val_loader, test_type="ConvNet", logger=logger)
...
def load_resized_data(args):
    ...
    if args.dataset == 'cifar10':
        ...
        train_dataset = datasets.ImageFolder(os.path.join(args.data_tiny_dir, "train"), transform=transforms.ToTensor()) # no augmentation
        val_dataset = datasets.ImageFolder(os.path.join(args.data_tiny_dir, "val"), transform=transform)
...
def condense(args, logger, device='cuda'):
    ...
    if not args.test:
        synset.test(args, val_loader, logger, bench=True)
    ...
    it_log = n_iter // 50
    it_test = [200, n_iter // 5, 800,  n_iter // 2, 1200, 1500, n_iter]
    # it_test = [40, 80]
    ...
    for it in range(n_iter):
        ...
        for ot in range(args.inner_loop):
            ...
            net_res = get_network("ResNet18", 3, 10, (32, 32)).to(device) 
            # get a random model
            ...
            for c in range(nclass):
                ...
                # CIFAR100 ipc=10 100
                # CIFAR100 ipc=50 50
                features = embed_res(img_syn)
                prob = F.softmax(features, 1)
                syn_centroids = torch.mean(prob, 0)
                syn_centroids = syn_centroids.log()[None, :].expand(features.shape[0], -1)
                loss_cmi = F.kl_div(syn_centroids.reshape(features.shape[0], -1), F.log_softmax(features, 1).reshape(features.shape[0], -1), \
                        reduction="batchmean", log_target=True) * 100
        ...
        # Logging
        if it % it_log == 0:
            logger(f"{utils.get_time()} (Iter {it:3d}) loss: {loss_total/nclass/args.inner_loop:.1f}")
        if (it + 1) in it_test:
