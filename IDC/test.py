...
def alexnet(args, nclass, logger=None):
    model = ALEX.AlexNet(args.nch, nclass)
    # model = ALEX.AlexNetBN(args.nch, nclass)
    if logger is not None:
        logger(f"=> creating model alexnet")
    return model

def vgg11(args, nclass, logger=None):
    model = VGG.VGG11(args.nch, nclass)
    # model = VGG.VGG11BN(args.nch, nclass)
    if logger is not None:
        logger(f"=> creating model vgg11")
    return model
...
def test_data(args,
    ...
    if model_fn is None:
        ...
        if test_type == "ResNet":
            model_fn_ls = [resnet18_bn]
        if test_type == "AlexNet":
            model_fn_ls = [alexnet]
        if test_type == "VGG11":
            model_fn_ls = [vgg11]
    else:
